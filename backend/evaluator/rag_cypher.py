...
def generate_response(dataset_path: str, store_path: str=None) -> pd.DataFrame: 
  ...
  if store_path:
    ...
    if all(col in cypher_dataset.columns.to_list() for col in ['question', 'cypher_gt', 'cypher_gen', 'answer']): 
  ...
  for index, row in cypher_dataset.iterrows():
    ...
    if is_valid_cypher(query=cypher): 
      ...
      try: 
        ...
        logger.error(f"Error during get response from chain cypher for query: {question}. {str(e)}")
  ...
  logger.info(f"Percent sucess evaluate cypher query: {total_sucess/len(cypher_dataset):.2f}")
  if store_path: 
...
def evaludate_cypher_rag(cypher_dataset: pd.DataFrame, store_path: str=None): 
  ...
  for index, row in tqdm(cypher_dataset.iterrows(), total=len(cypher_dataset), desc="Evaluating Cypher queries"):
    try:
      ...
      jacc = len(response_cypher_gt & response_cypher_pred) / len(response_cypher_gt | response_cypher_pred) if (response_cypher_gt | response_cypher_pred) else 1.0
...
if __name__ == "__main__": 
  cypher_dataset = generate_response(dataset_path=CYPHER_DATASET_EVAL_PATH ,
                                     store_path=CYPHER_RESULT_EVAL_PATH)
  cypher_eval = evaludate_cypher_rag(cypher_dataset=cypher_dataset, 
                                     store_path=CYPHER_RESULT_EVAL_PATH)
