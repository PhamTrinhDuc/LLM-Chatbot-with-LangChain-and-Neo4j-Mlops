...
# from langchain.chains.graph_qa.cypher import CYPHER_GENERATION_PROMPT
...
def create_retriver_cypher(temperature: int=0, 
                           top_k: int=10,
  ...
  cypher_generate_prompt = PromptTemplate(
    input_variables=["schema", "question"],
    template=cypher_generate_template
  )

  hospital_cypher_chain = GraphCypherQAChain.from_llm(
    cypher_llm=ChatOpenAI(model=HOSPITAL_CYPHER_MODEL, 
                          temperature=temperature), 
    qa_llm=ChatOpenAI(model=HOSPITAL_QA_MODEL, temperature=temperature), 
    graph=graph, 
    allow_dangerous_requests=True,
    verbose=True, # Whether intermediate steps your chain performs should be printed.
    qa_prompt=qa_generation_prompt, 
    cypher_prompt=cypher_generate_prompt, 
    validate_cypher=True,  #  If true, the Cypher query will be inspected for errors and corrected before running.
    top_k=top_k
  )
