...
meter = metrics.get_meter(name="chatbot.metrics", version="1.0.0")
...
chat_requests_total = meter.create_counter(
    name="chat_requests_total", description="Total number of chat requests", unit="1"
)
...
active_chat_sessions = meter.create_up_down_counter(
    name="active_chat_sessions", description="Number of active chat sessions", unit="1"
)
...
def track_tokens_usage(result: dict, endpoint: str):
    ...
    if "token_usage" in result:
        ...
        if input_tokens > 0:
            chat_token_total.add(input_tokens, {"endpoint": endpoint, "type": "input"})

        # Output tokens
        output_tokens = result["token_usage"].get("output", 0)
        if output_tokens > 0:
            chat_token_total.add(
                output_tokens, {"endpoint": endpoint, "type": "output"}
            )
...
def setup_metrics(app):
    ...
    try:
        ...
        FastAPIInstrumentor.instrument_app(
            app, excluded_urls="/metrics,/health"  # Không track các endpoints này
        )
