...
from prompt.evaluate import SYSTEM_CYPHER_GENERATION_TEMPLATE, USER_CYPHER_GENERATION_TEMPLATE

# Kết nối tới Neo4j
driver = GraphDatabase.driver(uri=AppConfig.NEO4J_URI, 
...
TOPICS_SPECIFICS = {
  "Hospital Analytics": ["Hospital", "Physician", "Visit"], # Quản trị & Hiệu suất Bệnh viện
  "Patient Journey": ["Patient", "Visit", "Hospital", "Physician"], # Hành trình & Hồ sơ Bệnh nhân
  "Healthcare Economics": ["Visit", "Payer", "Physician"], # Tài chính & Bảo hiểm
  "Advanced Reasoning": [], 
  "Physician Career & Background": ["Physician", "Hospital"], # Phân tích Sự nghiệp & Nhân sự
  "Admission Efficiency": ["Visit", "Hospital", "Patient"],  # Quy trình Nhập viện & Hiệu quả Vận hành
  "Data Validation & Integrity": ["Review", "Visit", "Physician", "Patient"], # Đối soát Dữ liệu & Tính nhất quán
...
def generate_cypher_query(schema_text: str, 
                          topic: str, 
                          focus_nodes: Optional[List[str]]=None,
                          num_pairs: int=5):
  
  system_prompt = SYSTEM_CYPHER_GENERATION_TEMPLATE.format(schema=schema_text) 
  user_prompt = USER_CYPHER_GENERATION_TEMPLATE.format(topic=topic, focus_nodes=focus_nodes, num_pairs=num_pairs)
...
if __name__ == "__main__":
  ...
  for i, (topic, focus_node) in enumerate(TOPICS_SPECIFICS.items()):
    data = generate_cypher_query(schema_text=schema_info, 
                                topic=topic, 
                                focus_nodes=focus_node)
