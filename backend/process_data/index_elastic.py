...
class ElsIndexer: 
  ...
  def _get_embeddings(self, text: Union[str, List[str]]) -> Union[List[float], List[List[float]]]:
    ...
    if self.model_name == "openai":
      ...
      if response.status_code == 200:
          ...
          raise Exception(f"Failed to get embeddings from HF API. Status: {response.status_code}, Response: {response.text}")
  ...
  def create_index(self): 
    ...
    try: 
      if not self.client.indices.exists(index=self.index_name):
        ...
        logger.info(f"Index name {self.index_name} already exists. Skip create index")
  ...
  def _proces_batch(self, chunks: list[dict], start_id: int):
    ...
    try:
      ...
      # failed = True
      if failed: 
        logger.error(f"Failed to index {len(failed)} documents: {failed[:3]}...")
    except Exception as e: 
