...
def test_dsm5_hybrid_search_endpoint_exists(client: TestClient):
    ...
    response = client.post(
        "/dsm5/hybrid",
        json={"query": "major depressive disorder", "top_k": 5}
    )
    assert response.status_code != 404
...
def test_dsm5_criteria_search_endpoint_exists(client: TestClient):
    ...
    response = client.get(
        "/dsm5/criteria",
        params={"disorder": "Major Depressive Disorder"}
    )
    assert response.status_code != 404
...
def test_dsm5_criteria_search_with_disorder(client: TestClient):
    ...
    response = client.get(
        "/dsm5/criteria",
        params={"disorder": "Generalized Anxiety Disorder"}
    )
...
def test_dsm5_search_response_format(client: TestClient):
    ...
    response = client.post(
        "/dsm5/search",
        json={"query": "autism spectrum disorder"}
    )
